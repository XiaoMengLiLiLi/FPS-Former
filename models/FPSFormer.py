...
from models.FPSAttn import FPSAttn
...
class GDFN_FeedForward(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias):
        super(GDFN_FeedForward, self).__init__()
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type,patch_size,pyramid_levels):
        super(TransformerBlock, self).__init__()

        self.norm1 = LayerNorm(dim, LayerNorm_type)
        self.attn = FPSAttn(heads=num_heads, n_rounds=2, channels=dim,patch_size=(patch_size,patch_size),r=1, pyramid_levels = pyramid_levels)
    ...
    def __init__(self,
        ...
        self.encoder_level1 = nn.Sequential(*[
            TransformerBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                             LayerNorm_type=LayerNorm_type, patch_size=patch_size, pyramid_levels=pyramid_levels) for i in range(num_blocks[0])])
        ...
        self.encoder_level2 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type,patch_size=patch_size, pyramid_levels=pyramid_levels) for i in range(num_blocks[1])])

        self.down2_3 = Downsample(int(dim * 2 ** 1))  ## From Level 2 to Level 3
        self.encoder_level3 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type,patch_size=patch_size, pyramid_levels=pyramid_levels) for i in range(num_blocks[2])])

        self.down3_4 = Downsample(int(dim * 2 ** 2))  ## From Level 3 to Level 4
        self.latent = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 3), num_heads=heads[3], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type,patch_size=patch_size, pyramid_levels=pyramid_levels) for i in range(num_blocks[3])])
        ...
        self.decoder_level1 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type,patch_size=patch_size, pyramid_levels=pyramid_levels) for i in range(num_blocks[0])])
